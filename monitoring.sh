...
network_monitoring() {
    ...
    ifconfig eth0 2>/dev/null | awk '/RX bytes/ {print "In: " $3/1024/1024 " MB, Out: " $7/1024/1024 " MB"}'
}
...
# Main script to handle arguments
if [ "$1" == "-realtime" ]; then
    while true; do
        clear
        top_10_apps
    ...
    case "$1" in
        ...
        *)
            echo "Usage: $0 {-cpu|-network|-disk|-load|-memory|-process|-services|-realtime}"
            exit 1
        ;;
    esac
fi
